    ...
    const warningMessages = computed(() => {
      ...
      if (props.mint.state === 'WARN' || props.mint.state === 'ERROR') {
        ...
        if (successfulRecentSwaps.length < props.requiredSuccessfulSwaps) {
          warningsNeeded.push(`We do not have enough recent data on this mint. There were only ${successfulRecentSwaps.length} successful ${successfulRecentSwaps.length === 1 ? 'swap' : 'swaps'} in the last ${props.recentDaysThreshold} days.`);
        }
